...
async function loadUsers() {
    try {
        ...
        const select = document.getElementById('recruiter_email');
        select.innerHTML = '<option value="">Select Recruiter</option>' + 
            recruiters.map(r => `<option value="${r.userid}">${r.userid}</option>`).join('');
    } catch (error) {
...
document.getElementById('create-profile-form').addEventListener('submit', async (e) => {
    ...
    const recruiter_email = document.getElementById('recruiter_email').value;
    ...
    try {
        await apiCall('/api/create_profile', {
            method: 'POST',
            body: JSON.stringify({ recruiter_email, company_name, designation })
        });
...
async function loadProfiles() {
    try {
        ...
        if (profiles.length === 0) {
            tbody.innerHTML = '<tr><td colspan="5" style="text-align: center;">No profiles yet</td></tr>';
            return;
        }
